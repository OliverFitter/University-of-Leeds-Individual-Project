...
Composite_properties=[('M40JB',222000000000.0,7010000000.0,0.314,4661000000.0,4661000000.0,4661000000.0,0.00004,1465.0,1644000000.0,1052000000.0,23000000.0,117000000.0,88263000.0,88263000.0),
    ('T300',101700000000.0,6000000000.0,0.3,2400000000.0,2400000000.0,2400000000.0,0.000029,1400.0,1820000000.0,1470000000.0,76000000.0,76000000.0,95000000.0,95000000.0,)]
def Createcomposite(mat_name, E_1, E_2, v, G_12, G_13, G_23, thickness, density, long_ten_strength, long_comp_strength, tran_ten_strength, tran_comp_strength, long_shear_strength, tran_shear_strength):
    mdb.models['Model-1'].Material(name=mat_name)
    mdb.models['Model-1'].materials[mat_name].Elastic(table=((
        E_1, E_2, v, G_12, G_13, G_23), ), type=LAMINA)
    mdb.models['Model-1'].materials[mat_name].Density(table=((density, ), 
...
for i in Composite_properties:
        Createcomposite(i[0], i[1], i[2], i[3], i[4], i[5], i[6], i[7], i[8], i[9], i[10], i[11], i[12], i[13], i[14])


# 5: Creates list of possible ply-layups for uniirectional composite with optimisation conditions
...
# 6: skin layup discrete optimisation code
# Displacement and failure criterion limits
skin_sets_list = [('skin mid','Skin-mid'),
    ('skin tip','Skin-tip'),
    ('skin-root-NS','skin-root-NS'),
    ('Singularity-skin','Singularity-skin'),
    ('SCB-NS','SCB-NS'),
    ('SCT-NS','SCT-NS'),
    ('SCB-Singularity','SCB-Singularity'),
    ('SCT-Singularity','SCT-Singularity'),
    ('spar cap bottom mid','Spar cap bottom mid'),
    ('spar cap top mid','Spar cap-top mid'),
    ('spar cap bottom tip','Spar cap-bottom tip'),
    ('spar cap top tip','Spar cap-top tip')]
displacement_threshold = 0.1753
tsaiwu_threshold = 0.6
max_displacement_values = []
Mass = []
max_TSAIW = []

file_name = "results_M.csv"
# Creates csv file to save results
with open(file_location + file_name, 'wb') as csvfile:
    writer = csv.writer(csvfile)
    # Write the header row
    header = ['Test Layup', 'Mass', 'Max TsaiWu', 'Max Displacement']
    writer.writerow(header)  
    # Part of the stopping condition
    continue_testing = True
    current_nr_of_layers = 1
    stop_nr_of_layers = -1
    successful_nr_of_layers = -1
    while continue_testing:
        combinations_to_test = [i for i in range(len(test_layups)) if len(test_layups[i]) == current_nr_of_layers]
        for test_layup_iteration in combinations_to_test:
            stop_testing = False
            # For each layup deletes previous layup for skin components and adds in new layup
            for i in skin_sets_list:
                mdb.models['Model-1'].parts['Wing'].compositeLayups[i[0]].deletePlies()
            for section_name in skin_sets_list[4:]:
                mdb.models['Model-1'].parts['Wing'].compositeLayups[section_name[0]].CompositePly(
                    additionalRotationField='', additionalRotationType=ROTATION_NONE, angle=0.0,
                    axis=AXIS_3, material='Carbon Fibre-200GSM', numIntPoints=3,
                    orientationType=SPECIFY_ORIENT, orientationValue=45.0, plyName='Ply-a',
                    region=mdb.models['Model-1'].parts['Wing'].sets[section_name[1]], suppressed=False,
                    thickness=0.00032, thicknessType=SPECIFY_THICKNESS)
                mdb.models['Model-1'].parts['Wing'].compositeLayups[section_name[0]].CompositePly(
                    additionalRotationField='', additionalRotationType=ROTATION_NONE, angle=0.0,
                    axis=AXIS_3, material='Carbon Fibre-200GSM', numIntPoints=3,
                    orientationType=SPECIFY_ORIENT, orientationValue=45.0, plyName='Ply-b',
                    region=mdb.models['Model-1'].parts['Wing'].sets[section_name[1]], suppressed=False,
                    thickness=0.00032, thicknessType=SPECIFY_THICKNESS)
                mdb.models['Model-1'].parts['Wing'].compositeLayups[section_name[0]].CompositePly(
                    additionalRotationField='', additionalRotationType=ROTATION_NONE, angle=0.0,
                    axis=AXIS_3, material='Carbon UD Tape', numIntPoints=3, orientationType=SPECIFY_ORIENT,
                    orientationValue=0.0, plyName='Ply-c', region=mdb.models['Model-1'].parts['Wing'].sets[section_name[1]],
                    suppressed=False, thickness=0.0003, thicknessType=SPECIFY_THICKNESS)
            for i in range(len(test_layups[test_layup_iteration])):
                angle = test_layups[test_layup_iteration][i]
                ply_name = "'Ply_" + str(i+1) + "'"
                for section_name in skin_sets_list:
                    mdb.models['Model-1'].parts['Wing'].compositeLayups[section_name[0]].CompositePly(
                        additionalRotationField='', additionalRotationType=ROTATION_NONE, angle=0.0,
                        axis=AXIS_3, material='T300', numIntPoints=3,
                        orientationType=SPECIFY_ORIENT, orientationValue=angle, plyName=ply_name,
                        region=mdb.models['Model-1'].parts['Wing'].sets[section_name[1]], suppressed=False,
                        thickness=2.9e-5, thicknessType=SPECIFY_THICKNESS)
            #creates and runs a job - has the same name for each iteration so that minimal space on computer is taken up
            job_name = 'Job-1'
            ...
            Mass.append(mdb.models['Model-1'].parts['Wing'].getMassProperties()['mass']) # Determines mass of each layup
            odb_path = file_location + job_name + ".odb"
            odb = session.openOdb(odb_path)          
            # Determines the max envelope Tsai Wu failure index for entire wing for each layup
            max_values_TSAIW = []
            for i in range(len(odb.steps['Step-1'].frames[-1].fieldOutputs['TSAIW'].bulkDataBlocks)):
                max_value_TSAIW = np.max(odb.steps['Step-1'].frames[-1].fieldOutputs['TSAIW'].bulkDataBlocks[i].data)
                max_values_TSAIW.append(max_value_TSAIW)
            overall_max_TSAIW = np.max(max_values_TSAIW)
            max_TSAIW.append(overall_max_TSAIW)
            print(str(test_layups[test_layup_iteration]))
            print(overall_max_TSAIW)
            # Determines maximum U2 displacement for each layup
            overall_max_values_Us=max(odb.steps['Step-1'].frames[-1].fieldOutputs['U'].bulkDataBlocks[0].data[:, 1])
            max_displacement_values.append(overall_max_values_Us)
            print(overall_max_values_Us)
            odb.close() # closes Job-1.odb so that job can be written over to save space           
            #writes the results into the csv file
            row = [str(test_layups[test_layup_iteration]), Mass[-1], max_TSAIW[-1], max_displacement_values[-1]]
            writer.writerow(row)
# Stopping condition - if both the max displacement and max failure index are below the desired limits 
#                      the code will stop once all the layups with the same number of plies as the 
#                      sucessful layup have been run. This ensures a layup with the same mass that has 
#                      a lower displacement and/or failure index is not missed.
            if max_displacement_values[-1] < displacement_threshold and max_TSAIW[-1] < tsaiwu_threshold:
                stop_testing = True
            if stop_testing:
                if stop_nr_of_layers == -1:
                    stop_nr_of_layers = current_nr_of_layers
                    successful_nr_of_layers = current_nr_of_layers
                elif current_nr_of_layers > stop_nr_of_layers:
                    if current_nr_of_layers > successful_nr_of_layers:
                        continue_testing = False
                    break
        current_nr_of_layers += 1
        if current_nr_of_layers > N:
            continue_testing = False